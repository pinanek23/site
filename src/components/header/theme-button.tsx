...
const ThemeButton: Component = () => {
  const [colorMode, setColorMode] = createSignal<'light' | 'dark' | 'none'>('none')
  ...
  function handleToggleTheme() {
    ...
    setColorMode((value) => {
      ...
      newValue === 'dark' ? root.classList.add('dark') : root.classList.remove('dark')
  ...
  onMount(() => {
    ...
    window.matchMedia('(prefers-color-scheme: dark)').addEventListener('change', handlePreferColorModeChange)
  })
  ...
  return (
    <Show
      when={colorMode() !== 'none'}
      fallback={
        <svg class={classnames(Styles.size, Styles.svg)} viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
